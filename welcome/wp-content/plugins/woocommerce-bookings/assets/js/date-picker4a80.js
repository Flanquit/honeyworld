...
jQuery( function( $ ) {
	var wc_bookings_timeout      = 0,
		...
		select_date_trigger: function( date, e, i, a ) {
			var fieldset          = $( this ).closest('fieldset'),
				picker            = fieldset.find( '.picker:eq(0)' ),
				form              = $( this ).closest( 'form' ),
            ...
            var selector = $( 'td[data-month="' + (parseInt(parsed_date[1]) - 1) + '"] a').filter(function() {
                return $(this).text() == parsed_date[2];
            }).parent();
			...
			setTimeout(function() {
                WC_DatePicker.applyStylesToDates(globalDateRange);
                selector.addClass('selected-date');
                selector.find('.placesLeft').css('display', 'block');
            }, 10);
		},

		date_picker_init: function( element ) {

            WC_DatePicker = new WC_Bookings_DatePicker( element );
			...
			WC_DatePicker.set_default_params({
				...
				onChangeMonthYear: function( year, month ) {
					...
					setTimeout(function () {
                        $('a.ui-state-highlight').removeClass('ui-state-highlight').removeClass('ui-state-active');
                    }, 100);
	...
	WC_Bookings_DatePicker.prototype.create = function create() {
        ...
        year = $('#initialYear').val();
        month = $('#initialMonth').val();
        day = 1;
	...
	WC_Bookings_DatePicker.prototype.applyStylesToDates = function applyStylesToDates( dateRange ) {
		...
		while ( checkDate < dateRange.endDate ) {
			...
			if ( ! this.cache.attributes[ cacheKey ] ) {
                ...
                var attributes	= this.getDateElementAttributes( checkDate, selector );
	 ...
	 */
	WC_Bookings_DatePicker.prototype.getDateElementAttributes = function getDateElementAttributes( date, element ) {
		...
		if ( this.bookingsData.partially_booked_days && this.bookingsData.partially_booked_days[ ymdIndex ] ) {
            ...
            let placesLeft = this.bookingsData.partially_booked_days[ ymdIndex ]['places_left'];
			let placesLeftText = '';
			if (typeof(getLangCode) !== 'undefined') {
				placesLeftText = getLangCode == 'en' ? placesLeft + ' tickets left' : ' נותרו ' + placesLeft + ' כרטיסים ';
				if (placesLeft == 1) {
					placesLeftText = getLangCode == 'en' ? placesLeft + ' ticket left' : 'נותר כרטיס אחד';
				}
			}
            attributes.placesleft = placesLeftText;
            attributes['data-placesleft'] = placesLeftText;
            attributes['data-placesleft-num'] = placesLeft;
            if (element)
				element.append('<div class="placesLeft">' + placesLeftText + '</div>');
		}
		...
		if ( ! bookable ) {
			...
			if ( attributes.class.indexOf( 'partial_booked' ) > -1 ) {
                ...
                let placesLeft = this.bookingsData.max_persons;
				let placesLeftText = '';
				if (typeof(getLangCode) !== 'undefined') {
					placesLeftText = getLangCode == 'en' ? placesLeft + ' tickets left' : ' נותרו ' + placesLeft + ' כרטיסים ';
					if (placesLeft == 1) {
